...
void ui_connect_stream(int appId)
{
  ...
  else
  {
    ...
    case STAGE_CONTROL_STREAM_START:
      stage = "Control stream start";
      break;
    case STAGE_VIDEO_STREAM_START:
      stage = "Video stream start";
      break;
    case STAGE_AUDIO_STREAM_START:
      stage = "Audio stream start";
      break;
    case STAGE_INPUT_STREAM_START:
      stage = "Input stream start";
      break;
    }
...
int ui_connect(char *name, char *address)
{
  ...
  if (!connection_is_ready())
  {
    ...
    ret = gs_init(&server, address, 47989, key_dir, 0, true);
...
device_info_t *ui_connect_and_pairing(device_info_t *info)
{
  ...
  int ret = gs_init(&server, info->internal, 47989, key_dir, 0, true);
...
bool check_connection(const char *name, char *addr)
{
  ...
  if (gs_init(&server, addr, 47989, key_dir, 0, true) != GS_OK)
  {
    return false;
  }
  connection_terminate();
